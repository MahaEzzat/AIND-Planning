...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #check for all actions if there are set of actions have preconditions and is subset of the current state.
        #if any add, add them to the action level and set the relation between the current state level and the current action nodes.
        for action in self.all_actions:    
            ...
            if (action_node.prenodes.issubset(S_Level)):
                ...
                for s_node in S_Level:
                    expration=expr(s_node.symbol)
                    if(s_node.is_pos):
                        if(expration in action_node.action.precond_pos):
                            s_node.children.add(action_node)
                            action_node.parents.add(s_node)
                    else:
                        if(expration in action_node.action.precond_neg):
                            s_node.children.add(action_node)
                            action_node.parents.add(s_node)
    ...
    def add_literal_level(self, level):
        ...
        #check for all action nodes in the prev. level if there are set of state nodes that are affected from the action nodes.
        #if any, add them to the state level and set the relation between the current state level and the prev. action level nodes.        
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return_value=True
        #check for every clause in node1 effect add, if it is not in node2 action preconditions pos.
        for clause in node_a1.action.effect_add:
            if clause not in node_a2.action.precond_pos:
                return_value=False
                break
            
        if(return_value):    
            #check for every clause in node1 effect rem, if it is not in node2 action preconditions neg.
            for clause in node_a1.action.effect_rem:
                if clause not in node_a2.action.precond_neg:
                    return_value=False
                    break                
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        """
        Test a pair of actions for mutual exclusion, returning True if the 
        effect of one action is the negation of a effect of the other.
        ...
        return_value=True
        #check for every clause in node1 effect add, if it is not in node2 action effect remove.
        for clause in node_a1.action.effect_add:
            if clause not in node_a2.action.effect_rem:
                return_value=False
                   
        if(not return_value): 
            #check for every clause in node2 effect add, if it is not in node1 action effect remove.
            for clause in node_a2.action.effect_add:
                if clause not in node_a1.action.effect_rem:
                    return_value=False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        #test if two nodes have the same expr but one pos and one neg.
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        #check if any action node can achieve the two nodes, and return false if any.#######
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        # TODO implement
        # for each goal in the problem, determine the level cost by searching in planning graph for this goal, then add levels together.
        ...
        for clause in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if(state.is_pos):
                        expration=expr(state.symbol)
                    else:
                        expration=expr('~{}'.format(state.symbol))
                    if clause == expration:
                        level_sum += level
                        goal_found=True
                        break
